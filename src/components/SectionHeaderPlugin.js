...
export const SectionHeaderPlugin = {
    commands: {
        createSectionHeader(editor, sourceMap) {
            ...
            const {node, source, field} = getTextNodeAndSource(value, path, sourceMap)

            // Find the parent of that json node 
            const sourceParentArray = getSourceParentArray(value, node, sourceMap)
            ...
            const transformedHeader = toSlateJson(headerJson, sourceMap)
